...
CameraPoseVisualization cameraPoseVisualization(0.5,0.2,0.3,0.8);
Eigen::Matrix3d rot_odom_cam;
void pubPayloadDroneStatus(nav_msgs::Odometry& odom, nav_msgs::Odometry& payload_odom )
{
  int i = 0;
  // drone visualization
  vis_msg_.markers[i].header.stamp = ros::Time::now();

  vis_msg_.markers[i].pose.orientation.w = odom.pose.pose.orientation.w;
  vis_msg_.markers[i].pose.orientation.x = odom.pose.pose.orientation.x;
  vis_msg_.markers[i].pose.orientation.y = odom.pose.pose.orientation.y;
  vis_msg_.markers[i].pose.orientation.z = odom.pose.pose.orientation.z;
  vis_msg_.markers[i].pose.position.x = odom.pose.pose.position.x;
  vis_msg_.markers[i].pose.position.y = odom.pose.pose.position.y;
  vis_msg_.markers[i].pose.position.z = odom.pose.pose.position.z;
  ++i;
  vis_msg_.markers[i].header.stamp = ros::Time::now();
  vis_msg_.markers[i].pose.orientation.w = payload_odom.pose.pose.orientation.w;
  vis_msg_.markers[i].pose.orientation.x = payload_odom.pose.pose.orientation.x;
  vis_msg_.markers[i].pose.orientation.y = payload_odom.pose.pose.orientation.y;
  vis_msg_.markers[i].pose.orientation.z = payload_odom.pose.pose.orientation.z;
  vis_msg_.markers[i].pose.position.x = payload_odom.pose.pose.position.x;
  vis_msg_.markers[i].pose.position.y = payload_odom.pose.pose.position.y;
  vis_msg_.markers[i].pose.position.z = payload_odom.pose.pose.position.z;
  ++i;
  vis_msg_.markers[i].header.stamp = ros::Time::now();
  vis_msg_.markers[i].points.clear();
  vis_msg_.markers[i].pose.orientation.w = 1;
  geometry_msgs::Point p;
  p.x = payload_odom.pose.pose.position.x;//pos_l_real(0);
  p.y = payload_odom.pose.pose.position.y;//pos_l_real(1);
  p.z = payload_odom.pose.pose.position.z;//pos_l_real(2);
  vis_msg_.markers[i].points.push_back(p);
  p.x = odom.pose.pose.position.x;//pos_real(0);
  p.y = odom.pose.pose.position.y;//pos_real(1);
  p.z = odom.pose.pose.position.z;//pos_real(2);
  vis_msg_.markers[i].points.push_back(p);

  payloadPub.publish(vis_msg_);
}
...
void odom_callback(const nav_msgs::Odometry::ConstPtr& msg)
{
  ...
  nav_msgs::Odometry tmp_odom = *msg;
  pubPayloadDroneStatus(tmp_odom,payload_odom_);
  Eigen::Vector3d pos_drone(msg->pose.pose.position.x,msg->pose.pose.position.y,msg->pose.pose.position.z);
  Eigen::Quaterniond ori_drone(msg->pose.pose.orientation.w,msg->pose.pose.orientation.x,msg->pose.pose.orientation.y,msg->pose.pose.orientation.z);
  ori_drone=ori_drone*rot_odom_cam;
  cameraPoseVisualization.reset();
  cameraPoseVisualization.add_pose(pos_drone,ori_drone);
  cameraPoseVisualization.publish_by(cameraPub,tmp_odom.header);
...
void payload_odom_callback(const nav_msgs::Odometry::ConstPtr& msg)
{
  payload_odom_ = *msg;
}
...
void initVisMarker(visualization_msgs::MarkerArray& vis )
{
  visualization_msgs::Marker meshROS;
  meshROS.header.frame_id = _frame_id;
  ...
  meshROS.type = visualization_msgs::Marker::MESH_RESOURCE;
  meshROS.action = visualization_msgs::Marker::ADD;
  meshROS.pose.position.x = 0;
  meshROS.pose.position.y = 0;
  ...
  meshROS.color.b = color_b;
  meshROS.mesh_resource = mesh_resource;
  vis.markers.push_back(meshROS);
  visualization_msgs::Marker payload = meshROS;
  ...
  vis.markers.push_back(payload_line);
}
int main(int argc, char** argv)
{
  ...
  n.param("payload_size_",   payload_size_, 0.15 ); 
  ...
  ros::Subscriber sub_cmd  = n.subscribe("cmd",  100,  cmd_callback,ros::TransportHints().tcpNoDelay());
  ros::Subscriber sub_payload  = n.subscribe("payload_odom",  100,  payload_odom_callback,ros::TransportHints().tcpNoDelay());
  ...
  payloadPub = n.advertise<visualization_msgs::MarkerArray>(        "payload_status",              100, true);  
  cameraPub = n.advertise<visualization_msgs::MarkerArray>(        "camera",              100, true); 
  tf::TransformBroadcaster b;
  broadcaster = &b;
  initVisMarker(vis_msg_);
  rot_odom_cam << 0, 0, 1, -1, 0, 0, 0, -1, 0;
  ros::spin();
