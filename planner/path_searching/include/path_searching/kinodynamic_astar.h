...
class KinodynamicAstar {
 private:
  ...
  Eigen::Matrix<double, 3, 3> phi_; // state transit matrix
  ...
  /* helper */
  Eigen::Vector3d cable_direction(Eigen::Vector3d acc)
  {
    Eigen::Vector3d Tp = acc;
    Tp(2) += gAcc_; //here is -Tp
    return Tp/Tp.norm();
  }
  Eigen::Vector3d drone_pos(Eigen::Vector3d pos, Eigen::Vector3d q, double L) //optimized
  {
    return pos + L*q;
  }
  Eigen::Vector3d drone_pos(Eigen::Vector3d pos, double L, Eigen::Vector3d acc)
  {
    return pos + L*cable_direction(acc);
  }
  ...
  bool computeShotTraj(Eigen::Matrix3d state1, Eigen::Matrix3d state2,
                       double time_to_goal);
  double estimateHeuristic(Eigen::Matrix3d x1, Eigen::Matrix3d x2, double& optimal_time);

  /* state propagation */
  void stateTransit(Eigen::Matrix3d& state0,
                    Eigen::Matrix3d& state1, Eigen::Vector3d um,
                    double tau);
 ...
 public:
  ...
  enum { REACH_HORIZON = 1, REACH_END = 2, NO_PATH = 3, NEAR_END = 4, TOO_SHORT = 5 };
  ...
  //            Eigen::Vector3d end_vel, Eigen::Vector3d end_acc,bool init, bool dynamic = false,
  //            double time_start = -1.0);
  int search(Eigen::Vector3d start_pt, Eigen::Vector3d start_vel,
             Eigen::Vector3d start_acc, Eigen::Vector3d end_pt,
             Eigen::Vector3d end_vel,bool init, bool dynamic = true,
             double time_start = -1.0);
  bool checkcollision(Eigen::Vector3d& coord, Eigen::Vector3d& q);

  void setEnvironment(const GridMap::Ptr& env);

  std::vector<Eigen::Vector3d> getKinoTraj(double delta_t);
  void getKinoTraj(double delta_t, vector<Eigen::Vector3d> &pos,vector<Eigen::Vector3d>& pos_drone);
  void getKinoNodesTraj(std::vector<Eigen::Vector3d>& state_list, std::vector<double>& t_list );

  void getSamples(double& ts, vector<Eigen::Vector3d>& point_set,
                  vector<Eigen::Vector3d>& start_end_derivatives);
  int KinoAstarSearchAndGetSimplePath(Eigen::Vector3d start_pt, Eigen::Vector3d start_vel,
             Eigen::Vector3d start_acc, Eigen::Vector3d end_pt,
